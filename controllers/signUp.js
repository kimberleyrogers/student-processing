...
router.post('/new_user', (req, res) => {
    ...
    console.log(`checking values recieved ${name} ${email} ${vtUsername} ${vtPassword}`)

    const vt_username_hash = generateHash(vtUsername)
    const vt_password_hash = generateHash(vtPassword)

    console.log(`checking values received ${name} ${email} ${vt_username_hash} ${vt_password_hash}`)

    if (!name || !email || !vtUsername || !vtPassword) {
        ...
        .then(dbResult => {
            if(dbResult.rows != "") {
                ...
                console.log('line 41')
                let sql = `
                    INSERT INTO users (name, email, vt_username_hash, vt_password_hash)
                    VALUES ($1, $2, $3, $4);
                `
                db.query(sql, [name, email, vt_username_hash, vt_password_hash])
                .then(dbResult => {
                    console.log('line 48')
                    res.json(dbResult)
                })
